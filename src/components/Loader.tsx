...
const renderImageBlocks = (image: LoadingImage) => (
  <ImageBlock variants={item} src={image.image} id={image.id} />
)
...
const Loader = ({ setLoading }: any) => {
  return (
    ...
    >
      ...
      <motion.div
        variants={item}
        className="absolute z-10 text-[60px] font-bold text-darkBrown px-[10px] bg-blue"
      >
        Hella Co.
      </motion.div>
    </motion.div>
...
export const ImageBlock = ({ variants, src, id }: any) => {
  return (
    <motion.div variants={variants} className={`absolute ${id}`}>
      <Image src={src} alt={`${id}`} width={220} height={200} />
    </motion.div>
  )
}
